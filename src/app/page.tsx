  ...
  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-800 antialiased selection:bg-primary selection:text-white overflow-x-hidden w-full">
      ...
      {activePage === "intro" && <section className="pt-4 pb-8 px-6 max-w-7xl mx-auto grid lg:grid-cols-12 gap-8 items-stretch print:hidden">
        ...
        <div className="lg:col-span-5 bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm space-y-6 flex flex-col justify-between">
          <div>
      ...
      {activePage === "calculator" && <section className="bg-slate-100/50 border-y border-slate-200/60 py-8 px-6 print:hidden">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 bg-white text-slate-850 p-8 rounded-3xl shadow-lg border border-slate-200/60">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out outline-none ${
                        buyOpd ? "bg-primary" : "bg-slate-200"
                      }`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          buyOpd ? "translate-x-5" : "translate-x-0"
                        }`}
                      />
                    </button>
                  </div>
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="bg-white p-8 rounded-3xl border border-slate-200/80 shadow-lg space-y-6 animate-fadeIn">
                <h4 className="text-sm font-bold text-brand-dark flex items-center gap-2 border-b border-slate-200/65 pb-3">
                ...
                <div className="space-y-4 text-xs text-slate-650 font-medium">
                  {/* วงเงินสูงสุด */}
                  <div className="flex justify-between items-center bg-slate-50 p-3.5 rounded-xl border border-slate-100 shadow-sm">
                    <span className="text-slate-500 font-semibold flex items-center gap-1.5 text-[13px]">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                    {/* หมวด 1 ค่าห้อง */}
                    <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 shadow-sm space-y-1">
                      <span className="text-slate-400 font-bold block text-[10px] uppercase tracking-wider">🛌 ค่าห้องและค่าอาหาร (หมวด 1)</span>
                      <span className="font-extrabold text-slate-800 block text-xs">
                        {currentIpdPlan.room.toLocaleString()} บ./วัน
                      </span>
                      <span className="text-[9px] text-slate-450 block">สูงสุดไม่เกิน 60 วันต่อครั้ง</span>
                    </div>
                    
                    {/* หมวด 1 ICU */}
                    <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 shadow-sm space-y-1">
                      <span className="text-slate-400 font-bold block text-[10px] uppercase tracking-wider">🚨 กรณีรักษาในห้องผู้ป่วยวิกฤติ (ICU)</span>
                      <span className="font-extrabold text-slate-800 block text-xs">
                        {currentIpdPlan.icu.toLocaleString()} บ./วัน
                      </span>
                      <span className="text-[9px] text-slate-450 block">จ่าย 2 เท่า สูงสุดไม่เกิน 15 วัน</span>
                    </div>

                    {/* หมวด 2 ค่ารักษาพยาบาลทั่วไป */}
                    <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 shadow-sm space-y-1">
                      <span className="text-slate-400 font-bold block text-[10px] uppercase tracking-wider">🩺 ค่ารักษาพยาบาลทั่วไป (หมวด 2)</span>
                      <span className="font-extrabold text-slate-800 block text-xs">
                        {currentIpdPlan.medical.toLocaleString()} บ./ครั้ง
                      </span>
                      <span className="text-[9px] text-slate-450 block">ค่าบริการวินิจฉัย/แล็บ/โลหิต/บำบัด</span>
                    </div>

                    {/* หมวด 3 ค่าแพทย์ตรวจรักษา */}
                    <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 shadow-sm space-y-1">
                      <span className="text-slate-400 font-bold block text-[10px] uppercase tracking-wider">🩺 ค่าประกอบวิชาชีพแพทย์ตรวจ (หมวด 3)</span>
                      <span className="font-extrabold text-slate-800 block text-xs">
                        {currentIpdPlan.doctor.toLocaleString()} บ./วัน
                      </span>
                      <span className="text-[9px] text-slate-450 block">ตรวจรักษาผู้ป่วยใน สูงสุด 60 วัน</span>
                    </div>

                    {/* หมวด 4 ค่าผ่าตัด */}
                    <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 shadow-sm space-y-1">
                      <span className="text-slate-400 font-bold block text-[10px] uppercase tracking-wider">🔪 ค่าผ่าตัดและหัตถการ (หมวด 4)</span>
                      <span className="font-extrabold text-slate-800 block text-xs">
                        {currentIpdPlan.surgery.toLocaleString()} บ./ครั้ง
                      </span>
                      <span className="text-[9px] text-slate-450 block">ผ่าตัดใหญ่และ Day Surgery (ไม่ต้องค้างคืน)</span>
                    </div>

                    {/* หมวด 4.5 ค่ารักษาผ่าตัดเปลี่ยนอวัยวะ */}
                    <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 shadow-sm space-y-1">
                      <span className="text-slate-400 font-bold block text-[10px] uppercase tracking-wider">🔄 ค่าผ่าตัดเปลี่ยนอวัยวะ (หมวด 4.5)</span>
                      <span className="font-extrabold text-slate-800 block text-xs">
                        {(currentIpdPlan.surgery * 2).toLocaleString()} บ./ครั้ง
                      </span>
                      <span className="text-[9px] text-slate-455 block">จ่าย 2 เท่า ของผลประโยชน์การผ่าตัด</span>
                    </div>

                    {/* หมวด 7 ค่าอุบัติเหตุฉุกเฉิน OPD */}
                    <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 shadow-sm space-y-1">
                      <span className="text-slate-400 font-bold block text-[10px] uppercase tracking-wider">🩹 อุบัติเหตุฉุกเฉิน OPD 24 ชม. (หมวด 7)</span>
                      <span className="font-extrabold text-slate-800 block text-xs">
                        {currentIpdPlan.accident.toLocaleString()} บ./ครั้ง
                      </span>
                      <span className="text-[9px] text-slate-450 block">สำหรับการรักษาภายใน 24 ชม. หลังอุบัติเหตุ</span>
                    </div>

                    {/* หมวด 12 ค่าบริการรถพยาบาลฉุกเฉิน */}
                    <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 shadow-sm space-y-1">
                      <span className="text-slate-400 font-bold block text-[10px] uppercase tracking-wider">🚑 ค่ารถพยาบาลฉุกเฉิน (หมวด 12)</span>
                      <span className="font-extrabold text-slate-800 block text-xs">
                        {currentIpdPlan.ambulance.toLocaleString()} บ./ครั้ง
                      </span>
                      <span className="text-[9px] text-slate-450 block">รถพยาบาลฉุกเฉินส่งตัวรับการรักษา</span>
                    </div>
                  </div>

                  {/* อบ. 2 ประกันอุบัติเหตุ */}
                  <div className="flex justify-between items-center bg-slate-50 p-3.5 rounded-xl border border-slate-100 shadow-sm text-xs mt-3">
                    <span className="text-slate-500 font-semibold flex items-center gap-1.5">
                      🎗️ ประกันภัยอุบัติเหตุส่วนบุคคล (อบ. 2):
                    </span>
                    <span className="font-extrabold text-slate-800">
                      100,000 บาท <span className="text-[10px] text-slate-450 font-normal">(กรณีเสียชีวิต/สูญเสียอวัยวะ)</span>
                    </span>
                  </div>

                  {/* OPD Option Add-on details if buyOpd is active */}
                  {buyOpd ? (
                    <div className="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100 space-y-2.5 animate-fadeIn">
                      <span className="text-[10px] text-emerald-700 font-bold uppercase tracking-wider block">
                        🩹 ความคุ้มครองผู้ป่วยนอกเสริม (OPD): {currentOpdPlan.name}
                      </span>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-semibold">วงเงินรักษาผู้ป่วยนอก:</span>
                ...
                <div className="pt-6 border-t border-slate-150 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                    ...
                    <button
                      ...
                      disabled={!isEligible}
                      className={`w-full sm:w-auto px-8 py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-1.5 ${
                        isEligible
        ...
        <div className="bg-white rounded-3xl border border-slate-200/60 shadow-md overflow-hidden">
          ...
          <div className="grid grid-cols-3 bg-slate-50 border-b border-slate-100">
            <button
              ...
              className={`py-4 px-2 text-xs sm:text-sm font-bold border-b-2 transition-all cursor-pointer ${
                activeInfoTab === "notes"
